...
@connect(
  ({
    ...
    scoutingDetail: { collections }
  }) => ({ selectData, showCollectionsModal, collections })
)
export default class AllCollection extends React.Component {
  ...
  handleCollectionFeature = (collections, from = 'feature') => {
    ...
    this.setState({
      ...
      hasFilter:
        from === 'feature'
          ? true
          : this.state.activeTime.y &&
            !this.state.activeTime.m &&
            !this.state.activeTime.d
          ? false
          : true
    })
    if (from === 'time') {
      // DetailAction.renderGoupCollectionForLookingBack(list);
      ViewAction.renderGroupCollections(list)
    }
    // DetailAction.setGroupCollectionActive(active);
    ViewAction.setMediaOrFeatureCollectActiveStyle(active.id, false)
  }
  ...
  setActive = val => {
    ...
    ViewAction.setMediaOrFeatureCollectActiveStyle(val.id, false)
  }
              ...
              return (
                <div
                  ...
                  className={`${styles.collectionItem} ${
                    this.state.active.id === data.id ? styles.activity : ''
                  }`}
                >
                  <div
                    onClick={() => {
  ...
  toggleSelectResource = data => {
    ...
    let selectData = tmpArr.filter(item => data.target_id === item.data.id)[0]
    this.setState({
      ...
      <div className={styles.collectionBox}>
        <div className={styles.containerHeader}>
          <div className={styles.filterAction}>
            <span className={styles.board_name}>{board.board_name}</span>
            {hasFilter && (
              <span className={styles.clearFilter} onClick={this.reset}>
                <MyIcon type="icon-zhongzhi" />
                重置
              </span>
            )}
            <span
              className={styles.exitFull}
              onClick={() => {
                onClose && onClose()
              }}
            >
            ...
            {Object.keys(timeTree).map(group => {
              return (
                <div key={group} className={styles.collectionGroup}>
                  <span className={styles.collectionGroupName}>
                    {data.find(d => d.id === group)?.name}
                  </span>
                  {Object.keys(timeTree[group]).map(year => {
                    return Object.keys(timeTree[group][year]).map(
                      (month, index) => {
                        return this.renderHoursForDays(
                          timeTree[group][year][month],
                          { group, year, month },
                          index
                        )
                      }
