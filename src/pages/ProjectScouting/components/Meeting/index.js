...
import { Input, Button, Avatar, Popover, Row, Col, Checkbox, message } from 'antd';
...
export default function Meettings(props) {
  ...
  const getUsersList = ()=>{
    Action.meetting.fetchUsers({board_id: props.board.board_id}).then(res => {
      let data = res.data;
      setBoardUsers(data);
    })
  }
  ...
  const onStart = ()=>{
    ...
    let param = {
      topic:`地图协作会议`+ dateFormat(new Date().getTime(), 'yyyy/MM/dd HH:mm') ,
  ...
  return (
    <div className={styles.meettingContainer}>
      <div className={styles.meettingHeader}>
        <div className={styles.meettingContainerTitle}>
          输入手机号邀请协作
        </div>
        <div className={styles.meettingPhoneNumber}>
          ...
          content={
            <Checkbox.Group onChange={addSelect} style={{width:"100%"}} value={selectUsers}>
              {boardUsers.map(item => {
                return (<div className={styles.border_user} key={item.user_id} onClick={()=> clickAddUser(item)}>
      ...
      <div className={styles.renderList}>
          {getUsersRender().map((item) => {
            return (<div className={`${styles.users} ${animateCss.animated} ${animateCss.fadeInUp}`} key={item.user_id}>
              {item._type === 'phone' ?
              <Avatar icon={<MyIcon type="icon-weidenglutouxiang"/>}
              ...
              <span>{item.name}</span>
              <span className={styles.removeUser} onClick={()=> removeUser(item)}><MyIcon type="icon-guanbi2"/></span>
            </div>)
          })}
      </div>
      <div className={styles.footer}>
        <Button type="primary" onClick={onStart}>发起协作</Button>
      </div>
    </div>
  )
}
