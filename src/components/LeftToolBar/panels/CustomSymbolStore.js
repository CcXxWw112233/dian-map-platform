...
export default class CustomSymbolStore extends React.Component {
  ...
  render() {
    ...
    return (
      <div
        className={`${styles.panel} ${styles.defineIcons}`}
      ...
      >
        <div
          ...
          <RenameModal visible={this.state.showRename} appendTo={this} dataSource={this.state.uploadingIcon}
            onOk={this.onUpload}
            onCancel={()=> {this.cancelUpload();console.log('取消编辑')}}
          />
              ...
              {dataIcons.map((item,index) => {
                return (
                  <Tooltip key={item.id} title={item.icon_name || '未设置名称'}>
                    <div className={`${styles.iconItem} ${removeActives.indexOf(item.id) !== -1 ? styles.activity :''}`} onClick={()=> this.setRemoveKey(item.id)}>
                      <div className={styles.iconItem_img}>
                        <img src={item.icon_url} alt=""/>
                      </div>
                      <div className={styles.iconItem_name}>{item.icon_name || 'unkown'}</div>
                    </div>
                  </Tooltip>
                )
              })}
        ...
        <div className={styles.someThingsBtn}>
            {isRemoveActive ?
              <Fragment>
                <Button type='default' onClick={()=> this.changeRemoveActive(false)}>
                  取消
                </Button>
                <Popconfirm
                title={`确定删除选择的${removeActives.length}个符号吗?`} icon={<QuestionCircleOutlined style={{ color: 'red' }} />}
              ...
              (
                <Fragment>
                  ...
                  <Button type='primary' onClick={() => this.changeRemoveActive(true)}>
                    编辑
                  </Button>
                </Fragment>
              )
