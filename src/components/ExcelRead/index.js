...
export default class ExcelRead extends React.Component {
  ...
  saveDatas = async () => {
    ...
    let promise = hasLocation.map((item) => {
      ...
      if (item.name) return ADD_COLLECTION(obj);
      ...
      >
          ...
          footer={
            <div style={{ textAlign: "right" }}>
              <span style={{ float: "left", marginLeft: "10px" }}>
                当前选定的分组：<a>{group.name}</a>
              </span>
              <Button type="default" onClick={() => this.closeAll()}>
                取消
              </Button>
              <Button
                type="primary"
                disabled={!hasLocation.length}
                onClick={this.saveDatas}
              >
                导入到分组
              </Button>
            </div>
        ...
        >
          <FormatAddress
            ...
            onEnterPosition={this.setDataPosition}
          />
        </Modal>
      </div>
    );
